...
def get_sessionid():
    ...
    return datetime.now().strftime('_%y%m%d_%H%M%S')+'_'+str(SESSION_UUID)
...
def parse(opt_path, is_train=True):
    ...
    path_task = os.path.join(opt['path']['root'], opt['task'],SESSION_ID+'_'+str(opt['tag']))
...
def find_last_checkpoint_cascade(save_dir, net_type='G'):
    """
    Args: 
        save_dir: model folder
        net_type: 'G' or 'D' or 'optimizerG' or 'optimizerD'
    ...
    if save_dir is not None:
        ...
        if file_list:
            iter_exist = []
            for file_ in file_list:
                iter_current = re.findall(r"(\d+)_{}".format(net_type), file_)
                print(file_)
                print(iter_current)
                iter_exist.append(int(iter_current[0]))
            init_iter = max(iter_exist)
            init_path = os.path.join(save_dir, '{}_{}'.format(init_iter, net_type))
    else:
        init_iter = 0
        init_path = None
    return init_iter, init_path
...
def save(opt):
    ...
    dump_path = os.path.join(opt_path_copy, filename+SESSION_ID+'_'+str(opt['tag'])+ext)
