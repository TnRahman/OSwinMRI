'''
# -----------------------------------------
Model SwinV2IM Cascade - V4 - Calgary Campinas Dataset
...
from models.weighting_params import DCWeights
...
class MRI_SwinV2IM_KS4_CC(ModelBase):

    def __init__(self, opt):
        super(MRI_SwinV2IM_KS4_CC, self).__init__(opt)
        # ------------------------------------
        # define network
        # ------------------------------------
        ...
        if self.opt_train['residual_DC']:
            ...
            for i in range(self.net_num):
                self.dcw.append(DCWeights().to(self.device))
   ...
   # ----------------------------------------
    ...
    def total_loss(self, current_step):
        ...
        if self.opt_train['cascade_loss_type'] == 'DS':
            ...
            self.H_c = self.H[:,::2,:,:] + 1j*self.H[:,1::2,:,:]
            self.E_c = self.E[:,::2,:,:] + 1j*self.E[:,1::2,:,:]
            self.H_k = torch.fft.fftn(self.H_c,dim=(-2,-1))
            self.E_k = torch.fft.fftshift(torch.fft.fftn(self.E_c,dim=(-2,-1)),dim=(-2,-1))
        ...
        # self.loss_perc = self.perceptual_lossfn(self.E_mag, self.H_mag)
        ...
        if mode == 'train':
            ...
            if self.use_patch:
                ...
                if self.patch_num == 'all':
                    ...
                    self.patch_overlap_h = 8
                    self.patch_overlap_w = 18

                    self.stride_h = self.patch_size - self.patch_overlap_h
                    self.stride_w = self.patch_size - self.patch_overlap_w
                    self.h_idx_list = list(np.arange(0, self.height-self.patch_size, self.stride_h, dtype=np.int)) + [self.height-self.patch_size]
                    self.w_idx_list = list(np.arange(0, self.width-self.patch_size, self.stride_w, dtype=np.int)) + [self.width-self.patch_size]
                    ...
                    for h_idx in self.h_idx_list:
                        for w_idx in self.w_idx_list:
                            ...
                            self.L_patch = self.L[..., h_idx:h_idx+self.patch_size, w_idx:w_idx+self.patch_size]
                    ...
                    self.E_out = self.E_model.div_(self.pw)

                    if self.opt_train['use_DC']:
                        self.E = data_consistency(self.E_out, self.K, self.mask, step=step, dc_step=dc_step)
                    else:
                        self.E = self.E_out
                ...
                if self.opt_train['use_DC']:
                    for i in range(self.net_num):
                        if self.opt_train['residual_DC']:
                            ...
                            self.E_out.append(data_consistency(self.nets[i](self.E_out[i]), self.K, self.mask, step=step, dc_step=dc_step))
                else:
                    for i in range(self.net_num):
            ...
            if self.opt_test['use_DC']:
                for i in range(self.net_num):
                    if self.opt_train['residual_DC']:
                        ...
                        self.E_out.append(data_consistency(self.nets[i](self.E_out[i]), self.K, self.mask))
            else:
                for i in range(self.net_num):
                    self.E_out.append(self.nets[i](self.E_out[i]))
            self.E = self.E_out[-1]
