...
def uniform1d(pattern_shape, factor,nlines=0,direction = "column"):
    """
    ...
    # factor = round((factor * pattern_shape[1])/2)
    if (nlines%2==0):
        factor1 = round((factor * pattern_shape[1])/2)
        factor2 = round((factor * pattern_shape[1])/2)
    else:
        factor1 = math.floor((factor * pattern_shape[1])/2)
        factor2 = math.ceil((factor * pattern_shape[1])/2)


    # Generating uniformly distributed indexes
    idx1 = np.random.choice(np.arange(int(pattern_shape[1]/2)-int(nlines/2), dtype=int), factor1, replace=False)
    idx2 = np.random.choice(np.arange((pattern_shape[1]/2)+int(nlines/2), pattern_shape[1],dtype=int), factor2, replace=False)
    # indexes = np.random.choice(np.arange(pattern_shape[1], dtype=int), factor, replace=False)
    idxs = np.concatenate((idx1,idx2))

    under_pattern = np.zeros(pattern_shape, dtype=bool)
    under_pattern[:, idxs] = True
...
def uniform2d(pattern_shape, factor):
    ...
    print(indexes_x.dtype)
    print(indexes_x.shape)
...
def uniform_pattern(pattern_shape,factor,center = True, dim="1D",radius_nlines = 5,direction = "column"):
    ...
    if center == False:
        ...
        if dim == "1D":
            ...
            pattern1 = uniform1d(pattern_shape,factor,radius_nlines,direction)
...
def gaussian2d(pattern_shape, factor, full_mask=None, center=None, cov=None):
    """
    Description: creates a 2D gaussian sampling pattern of a 2D image
    :param pattern_shape: shape of the desired sampling pattern.
    :param factor: sampling factor in the desired direction
    :param full_center: lines (1D) or points (2D) to reserve for center
    :param center: coordinates of the center of the Gaussian distribution
    :param cov: covariance matrix of the distribution
    :return: sampling pattern image. It is a boolean image
    """
    ...
    if full_mask is not None:
        factor = factor - full_mask.sum()
        assert factor > 0, 'Fully sampled center is too large for given acceleration'
    ...
    while (samples.shape[0] < factor):
        ...
        if full_mask is not None:
          up = np.zeros(pattern_shape, dtype=bool)
          up[samples[:, 0], samples[:, 1]] = True
          gdp = np.logical_and(up,np.logical_not(full_mask))
          samples = np.column_stack(np.where(gdp==True))
        if samples.shape[0] < factor:
            m *= 2
            continue
...
def gaussian_pattern(pattern_shape,factor, dim="1D", full_center = None, direction = "column",center=None, cov=None):
    """
    ...
    if dim == "1D":
        return gaussian1d(pattern_shape,factor,full_center,direction,center,cov)
    elif dim == "2D":
        if full_center is None:
            return gaussian2d(pattern_shape,factor,full_center,center,cov)
        else:
            full_mask = centered_circle(pattern_shape, full_center)
            pattern2 = gaussian2d(pattern_shape,factor,full_mask,center,cov)
            
            return np.logical_or(full_mask, pattern2)
    else:
        raise("Invalid option")
...
def poisson_disc_pattern(pattern_shape,center = True,radius = 5, k  = 6, r = 1.6):
    """
    Description: creates a uniformly distributed sampling pattern.
    :param pattern_shape: shape of the desired sampling pattern.
    :param center: boolean variable telling whether or not sample low frequencies
    ...
    sample point; Default = 5
    :param r: Minimum distance between samples. Default = 2
    :return: sampling pattern. It is a boolean image
    """
...
def get_poisson_disc_pattern(pattern_shape,center=True,rad=5,slices=1):
    ps1 = poisson_disc_pattern(pattern_shape,center=True,radius=rad)
    ps11 = ps1[np.newaxis,:,:]
    for i in range(slices-1):
        ps1 = poisson_disc_pattern(pattern_shape,center=True,radius=rad)
        ps11 = np.concatenate((ps11,ps1[np.newaxis,:,:]),axis=0)

    return ps11



def spiral_pattern1(pattern_shape,afactor,phase):
    # Base pattern stays within bounds of data space
    id_x = []
    id_y = []
    ...
    for theta in np.linspace(0,2.9*math.pi,num=P):
        r = ((theta)**2)
        id_x.append(int(round(r*math.cos(Q*theta-offset)+(pattern_shape[0]/2))))
        id_y.append(int(round(r*math.sin(Q*theta-offset)+(pattern_shape[1]/2))))

    pattern = np.zeros(pattern_shape, dtype=bool)
    pattern[np.array(id_x), np.array(id_y)] = True
    return pattern

def spiral_pattern2(pattern_shape,afactor, phase):
    # Base pattern goes outside bounds of data space
    id_x = []
    id_y = []
    N = pattern_shape[0]*pattern_shape[1]
    offset = phase
    ...
    for theta in np.linspace(0,4*math.pi,num=P):
        r = ((theta)**2)
        id_x.append(int(round(r*math.cos(Q*theta-offset)+pattern_shape[0])))
        id_y.append(int(round(r*math.sin(Q*theta-offset)+pattern_shape[1])))

    pattern = np.zeros((2*pattern_shape[0],2*pattern_shape[1]), dtype=bool)
    pattern[np.array(id_x), np.array(id_y)] = True

    pattern2 = pattern[pattern_shape[0]//2:(3*pattern_shape[0])//2,pattern_shape[1]//2:(3*pattern_shape[1])//2]

    return pattern2

def get_spiral_pattern(pattern_shape,afactor,slices=1,bounded=True):
    divs = slices
    if bounded:
        sp1 = spiral_pattern1(pattern_shape,afactor,0)
        sp11 = sp1[np.newaxis,:,:]
        for i in range(divs-1):
            sp1 = spiral_pattern1(pattern_shape,afactor,(np.pi*i/divs))
            sp11 = np.concatenate((sp11,sp1[np.newaxis,:,:]),axis=0)
    else:
        sp1 = spiral_pattern2(pattern_shape,afactor,0)
        sp11 = sp1[np.newaxis,:,:]
        for i in range(divs-1):
            sp1 = spiral_pattern2(pattern_shape,afactor,(np.pi*i/divs))
            sp11 = np.concatenate((sp11,sp1[np.newaxis,:,:]),axis=0)

    return sp11


def get_custom_pattern(pattern_shape,rad=1,slices=1):
    divs = slices
    ptrn1 = centered_circle(pattern_shape, rad)
    ptrn11 = ptrn1[np.newaxis,:,:]
    for i in range(divs-1):
        ptrn1 = centered_circle(pattern_shape, rad)
        ptrn11 = np.concatenate((ptrn11,ptrn1[np.newaxis,:,:]),axis=0)

    return ptrn11

def get_gaussian_pattern(pattern_shape,factor, dim="1D", full_center = 10, direction = "column",center=None, cov=None):

    if full_center is None:
        return gaussian_pattern(pattern_shape,factor,dim,full_center,direction,center,cov)
    else:
        pattern1 = gaussian_pattern(pattern_shape,factor,dim,full_center,direction,center,cov)
        pattern2 = centered_circle(pattern_shape, full_center)
        return np.logical_or(pattern1, pattern2)
...
if __name__ == '__main__':
    ...
    mask_type = '2D'
    acc = 0.2
    umask = gaussian_pattern(mask_shape,acc,'2D', full_center = 7)
    # np.save('G_2D20_C7_256_218.npy',umask)
    print(umask.sum()/(mask_shape[0]*mask_shape[1]))
    # umask2 = gaussian_pattern(mask_shape,acc,'2D', full_center = None)
    # print(umask2.sum()/(mask_shape[0]*mask_shape[1]))
    # # print(cov)
    # fig = plt.figure()
    # plt.imshow(umask,cmap='gray')
    # fig = plt.figure()
    # plt.imshow(umask2,cmap='gray')
    # plt.show()
